...
@given(parsers.parse("the ADF pipeline {pipeline_name} has been triggered with {parameters}"))
...
@when(parsers.parse("I poll the pipeline every {seconds:d} seconds until it has completed"))
def poll_adf_pipeline(context, seconds: int):
    """Poll the Azure Data Factory pipeline until it completes or times out.

    Args:
        context: Behave context object.
        seconds: The polling interval in seconds (integer).
    """
    polling2.poll(
...
@then(parsers.parse("the ADF pipeline {pipeline_name} has finished with state {state}"))
...
@then(parsers.parse("the ADF pipeline {pipeline_name} completed in less than {seconds:f} seconds"))
def check_adf_pipeline_completion_time(context, pipeline_name: str, seconds: float):
    """Check if the Azure Data Factory pipeline completed within the specified time.

    Args:
        context: Behave context object.
        pipeline_name: Name of the pipeline.
        seconds: The maximum allowed duration in seconds (float).
    """
