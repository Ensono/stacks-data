...
from stacks_data.pyspark.etl import (
    EtlSession,
...
def mock_etl_session(mock_adls_client, spark):
    with patch("stacks_data.pyspark.etl.EtlSession.get_spark_session_for_adls", return_value=spark), patch(
        "stacks_data.pyspark.etl.AdlsClient", return_value=mock_adls_client
    ):
...
@patch("stacks_data.azure.datalake.adls.AdlsClient.get_file_url")
...
def test_read_latest_rundate_data(spark, mock_adls_client, tmp_path):
    ...
    with patch(
        "stacks_data.azure.datalake.adls.AdlsClient.get_directory_contents", side_effect=mock_get_directory_contents
    ), patch("stacks_data.azure.datalake.adls.AdlsClient.get_file_url", side_effect=mock_get_file_url):
...
def test_transform_and_save_as_delta(spark, mock_adls_client, tmp_path):
    ...
    with patch("stacks_data.azure.datalake.adls.AdlsClient.get_file_url", return_value=expected_output_path):
