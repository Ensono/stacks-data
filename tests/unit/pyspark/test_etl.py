...
def mock_adls_client():
    with patch("stacks.data.azure.datalake.base.DataLakeServiceClient", autospec=True) as mock_service_client:
...
@patch("stacks.data.azure.datalake.adls.AdlsClient.get_file_url")
...
def test_read_latest_rundate_data(spark, mock_adls_client, tmp_path):
    ...
    with patch(
        "stacks.data.azure.datalake.adls.AdlsClient.get_directory_contents", side_effect=mock_get_directory_contents
    ), patch("stacks.data.azure.datalake.adls.AdlsClient.get_file_url", side_effect=mock_get_file_url):
...
def test_transform_and_save_as_delta(spark, mock_adls_client, tmp_path):
    ...
    with patch("stacks.data.azure.datalake.adls.AdlsClient.get_file_url", return_value=expected_output_path):
